...
export const templateChoices: prompts.Choice[] = [
  ...
  },
  {
    ...
    description:
      "CRUD App with table and forms built on template: Minimal. API layer uses tRPC with API Gateway and data layer uses DynamoDB",
    value: Template.CrudDynamo,
  },
  {
    title: "CRUD App with Aurora Serverless Postgres",
    description:
      "CRUD App with table and forms built on template: Minimal. API layer uses tRPC with API Gateway and data layer uses Aurora Serverless Postgres",
